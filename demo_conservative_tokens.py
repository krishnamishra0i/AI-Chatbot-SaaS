#!/usr/bin/env python3
"""
DEMONSTRATION: CONSERVATIVE TOKEN ADJUSTMENT SYSTEM
Shows how questions are analyzed and get much shorter token limits
"""
...
def analyze_question_context(question: str) -> dict:
    ...
    # Determine response parameters based on analysis - CONSERVATIVE VERSION
    base_tokens = 80  # Much more conservative minimum tokens
    temperature = 0.3  # Conservative default

    # Adjust for emotions - much shorter responses
    if 'greetings' in detected_emotions:
        base_tokens = 35   # Very short, friendly responses
    ...
    # Context-based adjustments for specific domains - much shorter
    if any(word in question_lower for word in ['lms', 'learning', 'course', 'education']):
        base_tokens = max(base_tokens, 200)  # Educational content - reasonable detail
        ...
        base_tokens = max(base_tokens, 280)  # Code explanations - focused detail
        temperature = 0.2  # Very precise for code
    elif any(word in question_lower for word in ['financial', 'money', 'budget', 'credit']):
        base_tokens = max(base_tokens, 240)  # Financial advice - balanced detail
        temperature = 0.3  # Balanced but conservative

    # Ensure reasonable bounds - much more conservative maximum
    max_tokens = max(30, min(base_tokens, 400))  # Between 30-400 tokens (much shorter!)
...
def main():
    """Demonstrate the conservative token adjustment system"""

    print("="*80)
    print("🧪 CONSERVATIVE TOKEN ADJUSTMENT SYSTEM DEMONSTRATION")
    print("="*80)

    # Test questions with different emotions and contexts
    test_questions = [
        # Greetings - should be very short
        "Hi there!",
        "Hello, how are you?",
        "Good morning!",

        # Excited questions - short to medium
        "Wow, this is amazing!",
        "That's fantastic! How does it work?",

        # Frustrated questions - medium
        ...
        # Educational/LMS questions - reasonable
        "How do I access my courses in LMS?",
        "What is Athena LMS?",

        # Financial questions - balanced
    ...
    for i, question in enumerate(test_questions, 1):
        ...
        if analysis['max_tokens'] <= 60:
            style = "Ultra Short & Concise"
        elif analysis['max_tokens'] <= 120:
            style = "Short & Direct"
        elif analysis['max_tokens'] <= 240:
            style = "Medium & Focused"
        else:
            style = "Long & Detailed"
    ...
    print("\n" + "="*80)
    print("✅ CONSERVATIVE TOKEN ADJUSTMENT ANALYSIS COMPLETE")
    print("="*80)
    ...
    token_ranges = {
        'Ultra Short (≤60)': sum(1 for a in analyses if a['max_tokens'] <= 60),
    ...
    print("🚀 Answers will now be significantly shorter and more to the point!")
